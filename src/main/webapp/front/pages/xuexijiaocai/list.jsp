          ...
          <form class="layui-form">
            <div class="layui-inline" style="margin-bottom: 10px;">
              ...
              <div class="layui-input-inline">
                <input type="text" name="jiaocaimingcheng" id="jiaocaimingcheng" placeholder="教材名称" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-bottom: 10px;">
              <label class="layui-form-label">教材类型</label>
              <div class="layui-input-inline">
                <input type="text" name="jiaocaileixing" id="jiaocaileixing" placeholder="教材类型" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
              ...
              <button v-if="isAuth('xuexijiaocai','新增')" @click="jump('../xuexijiaocai/add.jsp')" type="button" class="layui-btn btn-theme">
          ...
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../xuexijiaocai/detail.jsp?id='+item.id">
                <div class="posi-img">
                  <img :src="item.tupian?item.tupian.split(',')[0]:''" width="385px" height="220px" />
                </div>
                <div class="remark">
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#jiaocaimingcheng').val()) {
          param['jiaocaimingcheng'] = jquery('#jiaocaimingcheng').val() ? '%' + jquery('#jiaocaimingcheng').val() + '%' : '';
        }  
        if (jquery('#jiaocaileixing').val()) {
          param['jiaocaileixing'] = jquery('#jiaocaileixing').val() ? '%' + jquery('#jiaocaileixing').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('xuexijiaocai/list', 'get', param, function(res) {
